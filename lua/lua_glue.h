...
class LuaException : public Core::CoreException {
 public:
  LuaException(const std::string &error) : CoreException("Lua error: ") {
    std::stringstream ss;
    ss << error;
    msg += ss.str();
  }
...
static inline int lua_specialized_call(
    lua_State *L, std::function<std::string(const std::string &)> closure) {
  if (lua_gettop(L) != 1) {
...
static inline int lua_specialized_call(
    lua_State *L, std::function<std::string(void)> closure) {
  if (lua_gettop(L) != 0) {
...
class LuaSupport {
 public:
  ...
  inline operator lua_State *() { return m_L; }
  ...
  int call(const std::string &code) {
    ...
    if (status) throw LuaException(lua_tostring(m_L, -1));
...
struct LuaFunction {
  ...
  ~LuaFunction() { m_lua.remove(m_name); }
...
class LuaLibrary {
 public:
  ...
  void finalize_library() { m_lua.add_callback(m_name, &m_library_open); }
